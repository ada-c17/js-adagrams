...
describe("Adagrams", () => {
  ...
  describe("usesAvailableLetters", () => {
    it("returns true if the submitted letters are valid against the drawn letters", () => {
      ...
      const isValid = A.usesAvailableLetters(word, drawn);
  ...
  describe("scoreWord", () => {
    const expectScores = (wordScores) => {
      Object.entries(wordScores).forEach(([word, score]) => {
        expect(A.scoreWord(word)).toBe(score);
      });
    };
  ...
  describe("highestScoreFrom", () => {
    it("returns a hash that contains the word and score of best word in an array", () => {
      ...
      const correct = { word: "XXXX", score: A.scoreWord("XXXX") };

      expect(A.highestScoreFrom(words)).toEqual(correct);
    });
    ...
    describe("in case of tied score", () => {
      const expectTie = (words) => {
        const scores = words.map((word) => A.scoreWord(word));
      ...
      it("selects the word with 10 letters", () => {
        ...
        const correct = {
          word: "AAAAAAAAAA",
          score: A.scoreWord("AAAAAAAAAA"),
        };
        expectTie(words);

        expect(A.highestScoreFrom(words)).toEqual(correct);
        expect(A.highestScoreFrom(words.reverse())).toEqual(correct);
      });

      it("selects the word with fewer letters when neither are 10 letters", () => {
        const words = ["MMMM", "WWW"];
        const correct = { word: "WWW", score: A.scoreWord("WWW") };
        expectTie(words);
      ...
      it("selects the first word when both have same length", () => {
        ...
        const second = {
          word: "EEEEEEEEEE",
          score: A.scoreWord("EEEEEEEEEE"),
        };
        expectTie(words);

        expect(A.highestScoreFrom(words)).toEqual(first);
        expect(A.highestScoreFrom(words.reverse())).toEqual(second);
      });
    });
  });
});
