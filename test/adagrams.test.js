...
const adagramsTest = new Adagrams();
describe("Adagrams", () => {
  describe("drawLetters", () => {
    it("draws ten letters from the letter pool", () => {
      const drawn = adagramsTest.drawLetters();
  ...
  describe("usesAvailableLetters", () => {
    it("returns true if the submitted letters are valid against the drawn letters", () => {
      ...
      const isValid = adagramsTest.usesAvailableLetters(word, drawn);
  ...
  describe("scoreWord", () => {
    const expectScores = (wordScores) => {
      Object.entries(wordScores).forEach(([word, score]) => {
        expect(adagramsTest.scoreWord(word)).toBe(score);
      });
    };
  ...
  describe("highestScoreFrom", () => {
    it("returns a hash that contains the word and score of best word in an array", () => {
      ...
      const correct = { word: "XXXX", score: adagramsTest.scoreWord("XXXX") };

      expect(adagramsTest.highestScoreFrom(words)).toEqual(correct);
    });
    ...
    describe("in case of tied score", () => {
      const expectTie = (words) => {
        const scores = words.map((word) => adagramsTest.scoreWord(word));
      ...
      it("selects the word with 10 letters", () => {
        ...
        const correct = {
          word: "AAAAAAAAAA",
          score: adagramsTest.scoreWord("AAAAAAAAAA"),
        };
        expectTie(words);

        expect(adagramsTest.highestScoreFrom(words)).toEqual(correct);
        expect(adagramsTest.highestScoreFrom(words.reverse())).toEqual(correct);
      });

      it("selects the word with fewer letters when neither are 10 letters", () => {
        const words = ["MMMM", "WWW"];
        const correct = { word: "WWW", score: adagramsTest.scoreWord("WWW") };
        expectTie(words);
      ...
      it("selects the first word when both have same length", () => {
        ...
        const second = {
          word: "EEEEEEEEEE",
          score: adagramsTest.scoreWord("EEEEEEEEEE"),
        };
        expectTie(words);

        expect(adagramsTest.highestScoreFrom(words)).toEqual(first);
        expect(adagramsTest.highestScoreFrom(words.reverse())).toEqual(second);
      });
    });
  });
});
