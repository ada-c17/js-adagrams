...
class Model {
  ...
  nextRound() {
    ...
    this.letterBank = new Adagrams().drawLetters();
  ...
  playWord(word) {
    ...
    return new Adagrams().scoreWord(word);
  }

  _valid(word, letterBank = this.letterBank) {
    if (word.length < 1) return false;
    return new Adagrams().usesAvailableLetters(word, letterBank);
  }
  ...
  _bestPlay(round, player) {
    ...
    return new Adagrams().highestScoreFrom(plays);
  }
  ...
  _roundWinner(round) {
    ...
    const { word: winningWord } = new Adagrams().highestScoreFrom(
